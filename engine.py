...
consume `Simulation` as a plain object. Direct O(N^2) is the only solver
wired in for now; Barnes-Hut and GPU backends will implement the same
`accel_fn(positions, masses) -> accelerations` signature so they can be
